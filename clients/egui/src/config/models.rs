...
impl CuratedModel {
    pub fn new(name: impl Into<String>, provider: impl Into<String>, model_id: impl Into<String>) -> Self {
        Self {
            name: name.into(),
...
impl ModelsConfig {
    ...
    pub fn save(&self) -> Result<(), ModelConfigError> {
        let path = Self::config_path()
            .ok_or_else(|| ModelConfigError::Write("Could not determine config path".to_string()))?;
        
        // Ensure config directory exists
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| ModelConfigError::Write(format!("Failed to create config directory: {e}")))?;
        }
    ...
    pub fn add_curated_model(&mut self, model: CuratedModel) {
        ...
        let exists = self.models.curated.iter().any(|m| {
            m.provider == model.provider && m.model_id == model.model_id
        });
