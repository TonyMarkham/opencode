...
impl ProviderClient {
    ...
    pub async fn discover_models(
        ...
        let mut request = match provider_config.auth_type.as_str() {
            ...
            "header" => {
                let header_name = provider_config
                    .auth_header
                    .as_ref()
                    .ok_or_else(|| ProviderError::Auth("auth_header required for header auth type".to_string()))?;
            ...
            "query_param" => {
                let param_name = provider_config
                    .auth_param
                    .as_ref()
                    .ok_or_else(|| ProviderError::Auth("auth_param required for query_param auth type".to_string()))?;
...
fn parse_provider_response(
    ...
    let models_array = json
        ...
        .ok_or_else(|| ProviderError::Parse(format!("Models array not found at path: {}", format.models_path)))?;
